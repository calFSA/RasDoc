using MatrizDeRastreabilidade.API.Helpers;
using Microsoft.AspNetCore.Mvc;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace MatrizDeRastreabilidade.API.Controllers
{
    [Route("api/[controller]")]
    [ApiController]
    public class ColaboradorController : ControllerBaseHelper
    {
        public ColaboradorController(IServiceProvider serviceProvider) : base(serviceProvider)
        {
        }

        // GET: api/<ColaboradorController>
        [HttpGet]
        public IEnumerable<string> Get()
        {
            return new string[] { "value1", "value2" };
        }

        // GET api/<ColaboradorController>/5
        [HttpGet("{id}")]
        public string Get(int id)
        {
            return "value";
        }

        // POST api/<ColaboradorController>
        [HttpPost]
        public void Post([FromBody] string value)
        {
        }

        // PUT api/<ColaboradorController>/5
        [HttpPut("{id}")]
        public void Put(int id, [FromBody] string value)
        {
        }

        // DELETE api/<ColaboradorController>/5
        [HttpDelete("{id}")]
        public void Delete(int id)
        {
        }
    }
}
